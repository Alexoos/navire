...
#[ORM\Entity(repositoryClass: NavireRepostory::class)]
...
class Navire
{
    ...
    #[Assert\Range(
            min: 3,
            notInRangeMessage: 'Le nom doit être de minimum {{ min }} caractères',
    )]
    private ?string $nom = null;

    #[ORM\Column(name:'mmsi', length: 9)]
    #[Assert\Regex('[1-9][0-9]{8}', message: 'le numéro IMO doit être unique et composé de 9 chiffres sans commencer par 0')]
    ...
    #[ORM\JoinColumn(name:'idaishiptype' ,nullable: false)]
    ...
    #[ORM\OneToMany(mappedBy: 'escales', targetEntity: Escale::class, orphanRemoval: true)]
    ...
    public function removeEscale(Escale $escale): self
    {
        if ($this->escales->removeElement($escale)) {
            ...
            if ($escale->getEscales() === $this) {
                $escale->setEscales(null);
            }
        }
